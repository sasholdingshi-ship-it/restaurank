  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Mercurial</h1>
          <p className="text-sm text-gray-500">{ingredients.length} ingrédients</p>
        </div>
        <input
          type="text"
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-xs text-gray-500 uppercase">
              ...
              <th className="px-4 py-3 text-right">Prix net/kg</th>
              ...
              <tr key={ing.id} className="border-t border-gray-100 hover:bg-gray-50">
                {editing === ing.id ? (
                  <>
                    ...
                    <td className="px-4 py-2">
                      <input className="border rounded px-2 py-1 text-sm w-full" value={editData.supplier || ""} onChange={e => setEditData({ ...editData, supplier: e.target.value })} />
                    </td>
                    <td className="px-4 py-2">
                      <input type="number" step="0.01" className="border rounded px-2 py-1 text-sm w-20 text-right" value={editData.priceTtc ?? ""} onChange={e => setEditData({ ...editData, priceTtc: parseFloat(e.target.value) || null })} />
                    </td>
                    <td className="px-4 py-2">
                      <input type="number" step="0.01" className="border rounded px-2 py-1 text-sm w-20 text-right" value={editData.priceHt ?? ""} onChange={e => setEditData({ ...editData, priceHt: parseFloat(e.target.value) || null })} />
                    </td>
                    <td className="px-4 py-2">
                      <input type="number" step="0.001" className="border rounded px-2 py-1 text-sm w-16 text-right" value={editData.weight ?? ""} onChange={e => setEditData({ ...editData, weight: parseFloat(e.target.value) || null })} />
                    </td>
                    <td className="px-4 py-2 text-right font-mono">{editData.pricePerKg?.toFixed(2) ?? "—"}</td>
                    <td className="px-4 py-2">
                      <input type="number" step="0.01" className="border rounded px-2 py-1 text-sm w-16 text-right" value={editData.lossPercent ?? 0} onChange={e => setEditData({ ...editData, lossPercent: parseFloat(e.target.value) || 0 })} />
                    </td>
                  ...
                  <>
                    ...
                    <td className="px-4 py-2 text-center">
                      <button onClick={() => startEdit(ing)} className="text-blue-600 hover:text-blue-800 text-xs font-medium">Modifier</button>
                    </td>
                  </>
                )}
              </tr>
