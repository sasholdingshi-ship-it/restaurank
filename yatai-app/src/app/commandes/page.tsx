...
export default function CommandesPage() {
  ...
  const handleOcrUpload = async (file: File) => {
    ...
    formData.append("year", String(year)); formData.append("month", String(month)); formData.append("day", String(ocrDay))
    try {
      const res = await fetch("/api/ocr", { method: "POST", body: formData })
      const data = await res.json()
      if (data.entries) { setOcrResults(data.entries); setOcrNotes(data.notes || "") }
      else setOcrNotes(data.error || "Erreur OCR")
    } catch { setOcrNotes("Erreur réseau") }
    setOcrLoading(false)
  }
  ...
  const applyOcr = () => {
    ...
    for (const entry of ocrResults) {
      if (entry.productId && entry.quantity > 0) { const key = `${entry.productId}-${ocrDay}`; newGrid.set(key, entry.quantity); newDirty.add(key) }
    }
    setGrid(newGrid); setDirty(newDirty); setOcrResults(null); setShowOcr(false)
  }

  const getProductTotal = (productId: number) => { let total = 0; for (let d = 1; d <= daysInMonth; d++) total += grid.get(`${productId}-${d}`) || 0; return total }
  const getAnomaly = (productId: number, value: number): string => {
    const stat = stats.get(productId); if (!stat || !value || stat.avg === 0) return ""
    if (value > stat.avg * 3) return "bg-red-100 text-red-800"; if (value > stat.avg * 2) return "bg-orange-100 text-orange-800"; return ""
  }

  const activeProducts = products.filter(p => { for (let d = 1; d <= daysInMonth; d++) if (grid.has(`${p.id}-${d}`)) return true; return false })
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-3 items-center">
          ...
          <button onClick={() => setShowOcr(!showOcr)} className="bg-purple-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-purple-700">OCR Photo</button>
          {dirty.size > 0 && <button onClick={save} disabled={saving} className="bg-orange-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-orange-600 disabled:opacity-50">{saving ? "Sauvegarde..." : `Sauvegarder (${dirty.size})`}</button>}
        </div>
      </div>
      ...
      {showOcr && (
        <div className="bg-white rounded-xl shadow-sm border border-purple-200 p-5 mb-6">
          ...
          <div className="flex gap-4 items-end mb-4">
            <div><label className="block text-xs text-gray-500 mb-1">Jour</label>
            ...
            <div><label className="block text-xs text-gray-500 mb-1">Photo du bon</label>
              <input type="file" accept="image/*" capture="environment" onChange={e => { if (e.target.files?.[0]) handleOcrUpload(e.target.files[0]) }} className="text-sm" /></div>
          ...
          {ocrResults && (
            <div>
              <table className="w-full text-sm mb-3">
                <thead><tr className="text-xs text-gray-500 uppercase border-b"><th className="py-2 text-left">Réf</th><th className="py-2 text-left">Produit</th><th className="py-2 text-right">Quantité</th><th className="py-2 text-center">Confiance</th></tr></thead>
                <tbody>
                  {ocrResults.map((entry, i) => (
                    <tr key={i} className={`border-t ${entry.confidence === 'low' ? 'bg-red-50' : entry.confidence === 'medium' ? 'bg-yellow-50' : ''}`}>
                      ...
                      <td className="py-1.5 text-right"><input type="number" value={entry.quantity} onChange={e => { const nr = [...ocrResults]; nr[i] = { ...entry, quantity: parseFloat(e.target.value) || 0 }; setOcrResults(nr) }} className="w-20 text-right border rounded px-2 py-1 text-sm" /></td>
                      <td className="py-1.5 text-center"><span className={`text-xs px-2 py-0.5 rounded-full ${entry.confidence === 'high' ? 'bg-green-100 text-green-700' : entry.confidence === 'medium' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>{entry.confidence}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <div className="flex gap-3">
                <button onClick={applyOcr} className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-green-700">Appliquer au jour {ocrDay}</button>
                <button onClick={() => { setOcrResults(null); setShowOcr(false) }} className="text-gray-500 px-4 py-2 rounded-lg text-sm hover:bg-gray-100">Annuler</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-auto">
          <table className="text-xs">
            <thead><tr className="bg-gray-50">
              ...
              {Array.from({ length: daysInMonth }, (_, i) => <th key={i} className="px-1 py-2 text-center min-w-[36px]">{i + 1}</th>)}
            ...
            <tbody>
              {displayProducts.map(product => {
                const total = getProductTotal(product.id); const stat = stats.get(product.id)
                return (
                  <tr key={product.id} className="border-t border-gray-100 hover:bg-gray-50/50">
                    <td className="px-2 py-1 sticky left-0 bg-white font-medium truncate max-w-[140px]" title={product.name}>
                      <span className="text-gray-400 mr-1">{product.ref}</span>{product.name}
                    </td>
                    <td className="px-1 py-1 text-right font-mono text-gray-500">{product.priceHt?.toFixed(1) ?? "—"}</td>
                    {Array.from({ length: daysInMonth }, (_, i) => {
                      const day = i + 1; const val = grid.get(`${product.id}-${day}`) || 0; const anomaly = getAnomaly(product.id, val)
                      return <td key={day} className={`px-0 py-0 text-center ${anomaly}`}>
                        <input type="number" min={0} step="any" value={val || ""} onChange={e => setCell(product.id, day, parseFloat(e.target.value) || 0)}
